...
export default function UsersFollowersFollowing() {
  const { id } = useParams();
  ...
  const handleContact = async (userId) => {
    try {
      const response = await axiosInstance.get(
        `${userApiEndPoint.USERSFOLLOWERSFOLLOWING}/${userId}`
      );
      ...
      <div className="border rounded p-4 flex items-center gap-4">
        ...
        <div>
          ...
          <div className="flex gap-6 mt-2 text-sm">
            <span>
              <b>{usersProfile?.followers?.length || 0}</b> Followers
            </span>

            <span>
              <b>{usersProfile?.following?.length || 0}</b> Following
            </span>
          </div>
        </div>
      </div>
